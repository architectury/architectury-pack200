...
class AdaptiveCoding implements CodingMethod {
    ...
    private static void makeMetaCoding(AdaptiveCoding run, Coding dflt,
        ...
        for (;;) {
            ...
            bytes.write(Constants._meta_run + KX + 4*KBFlag + 8*ABDef);
    ...
    public static int parseMetaCoding(byte[] bytes, int pos, Coding dflt, CodingMethod res[]) {
        ...
        if (op < Constants._meta_run || op >= Constants._meta_pop)  return pos-1; // backup
        AdaptiveCoding prevc = null;
        for (boolean keepGoing = true; keepGoing; ) {
            keepGoing = false;
            assert(op >= Constants._meta_run);
            op -= Constants._meta_run;
            int KX = op % 4;
            int KBFlag = (op / 4) % 2;
            int ABDef = (op / 8);
            assert(ABDef < 3);
            ...
            if (BDef == 0 &&
                ((op = bytes[pos] & 0xFF) >= Constants._meta_run) && op < Constants._meta_pop) {
                pos++;
                keepGoing = true;
            } else if (BDef == 0) {
