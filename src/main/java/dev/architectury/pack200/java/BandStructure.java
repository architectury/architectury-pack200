...
class BandStructure {
    ...
    protected abstract ConstantPool.Index getCPIndex(byte tag);
    ...
    static {
        assert(basicCodings[Constants._meta_default] == null);
        assert(basicCodings[Constants._meta_canon_min] != null);
        assert(basicCodings[Constants._meta_canon_max] != null);
        Map<Coding, Integer> map = new HashMap<>();
        for (int i = 0; i < basicCodings.length; i++) {
            Coding c = basicCodings[i];
            if (c == null)  continue;
            assert(i >= Constants._meta_canon_min);
    ...
    public static int parseMetaCoding(byte[] bytes, int pos, Coding dflt, CodingMethod[] res) {
        if ((bytes[pos] & 0xFF) == Constants._meta_default) {
            res[0] = dflt;
            return pos+1;
        }
        int pos2;
        ...
        protected void readDataFrom(InputStream in) throws IOException {
            ...
            if (!canVaryCoding()) {
                ...
                if (XB < 0) {
                    ...
                    bandCoding = regularCoding;
                    metaCoding = noMetaCoding;
                } else if (XB == Constants._meta_default) {
                    bandCoding = regularCoding;
                    metaCoding = defaultMetaCoding;
                } else {
                    if (verbose > 2)
    ...
    class CPRefBand extends ValueBand {
        ...
        public CPRefBand(String name, Coding regularCoding, byte cpTag, boolean nullOK) {
            super(name, regularCoding);
            this.nullOK = nullOK;
            if (cpTag != Constants.CONSTANT_None)
        ...
        public CPRefBand(String name, Coding regularCoding, Object undef) {
            this(name, regularCoding, Constants.CONSTANT_None, false);
        }
    ...
    static final byte[] defaultMetaCoding = { Constants._meta_default };
    ...
    CPRefBand cp_String = cp_bands.newCPRefBand("cp_String", UDELTA5, Constants.CONSTANT_Utf8);
    CPRefBand cp_Class = cp_bands.newCPRefBand("cp_Class", UDELTA5, Constants.CONSTANT_Utf8);
    CPRefBand cp_Signature_form = cp_bands.newCPRefBand("cp_Signature_form", Constants.CONSTANT_Utf8);
    CPRefBand cp_Signature_classes = cp_bands.newCPRefBand("cp_Signature_classes", UDELTA5, Constants.CONSTANT_Class);
    CPRefBand cp_Descr_name = cp_bands.newCPRefBand("cp_Descr_name", Constants.CONSTANT_Utf8);
    CPRefBand cp_Descr_type = cp_bands.newCPRefBand("cp_Descr_type", UDELTA5, Constants.CONSTANT_Signature);
    CPRefBand cp_Field_class = cp_bands.newCPRefBand("cp_Field_class", Constants.CONSTANT_Class);
    CPRefBand cp_Field_desc = cp_bands.newCPRefBand("cp_Field_desc", UDELTA5, Constants.CONSTANT_NameandType);
    CPRefBand cp_Method_class = cp_bands.newCPRefBand("cp_Method_class", Constants.CONSTANT_Class);
    CPRefBand cp_Method_desc = cp_bands.newCPRefBand("cp_Method_desc", UDELTA5, Constants.CONSTANT_NameandType);
    CPRefBand cp_Imethod_class = cp_bands.newCPRefBand("cp_Imethod_class", Constants.CONSTANT_Class);
    CPRefBand cp_Imethod_desc = cp_bands.newCPRefBand("cp_Imethod_desc", UDELTA5, Constants.CONSTANT_NameandType);
    IntBand   cp_MethodHandle_refkind = cp_bands.newIntBand("cp_MethodHandle_refkind", DELTA5);
    CPRefBand cp_MethodHandle_member = cp_bands.newCPRefBand("cp_MethodHandle_member", UDELTA5, Constants.CONSTANT_AnyMember);
    CPRefBand cp_MethodType = cp_bands.newCPRefBand("cp_MethodType", UDELTA5, Constants.CONSTANT_Signature);
    CPRefBand cp_BootstrapMethod_ref = cp_bands.newCPRefBand("cp_BootstrapMethod_ref", DELTA5, Constants.CONSTANT_MethodHandle);
    IntBand   cp_BootstrapMethod_arg_count = cp_bands.newIntBand("cp_BootstrapMethod_arg_count", UDELTA5);
    CPRefBand cp_BootstrapMethod_arg = cp_bands.newCPRefBand("cp_BootstrapMethod_arg", DELTA5, Constants.CONSTANT_LoadableValue);
    CPRefBand cp_InvokeDynamic_spec = cp_bands.newCPRefBand("cp_InvokeDynamic_spec", DELTA5, Constants.CONSTANT_BootstrapMethod);
    CPRefBand cp_InvokeDynamic_desc = cp_bands.newCPRefBand("cp_InvokeDynamic_desc", UDELTA5, Constants.CONSTANT_NameandType);
    ...
    CPRefBand attr_definition_name = attr_definition_bands.newCPRefBand("attr_definition_name", Constants.CONSTANT_Utf8);
    CPRefBand attr_definition_layout = attr_definition_bands.newCPRefBand("attr_definition_layout", Constants.CONSTANT_Utf8);
    ...
    CPRefBand ic_this_class = ic_bands.newCPRefBand("ic_this_class", UDELTA5, Constants.CONSTANT_Class);
    IntBand ic_flags = ic_bands.newIntBand("ic_flags", UNSIGNED5);
    // These bands contain data only where flags sets ACC_IC_LONG_FORM:
    CPRefBand ic_outer_class = ic_bands.newCPRefBand("ic_outer_class", DELTA5, Constants.CONSTANT_Class, NULL_IS_OK);
    CPRefBand ic_name = ic_bands.newCPRefBand("ic_name", DELTA5, Constants.CONSTANT_Utf8, NULL_IS_OK);

    // bands for carrying class schema information:
    MultiBand class_bands = all_bands.newMultiBand("(class_bands)", DELTA5);
    CPRefBand class_this = class_bands.newCPRefBand("class_this", Constants.CONSTANT_Class);
    CPRefBand class_super = class_bands.newCPRefBand("class_super", Constants.CONSTANT_Class);
    IntBand   class_interface_count = class_bands.newIntBand("class_interface_count");
    CPRefBand class_interface = class_bands.newCPRefBand("class_interface", Constants.CONSTANT_Class);
    ...
    CPRefBand field_descr = class_bands.newCPRefBand("field_descr", Constants.CONSTANT_NameandType);
    ...
    CPRefBand field_ConstantValue_KQ = field_attr_bands.newCPRefBand("field_ConstantValue_KQ", Constants.CONSTANT_FieldSpecific);
    CPRefBand field_Signature_RS = field_attr_bands.newCPRefBand("field_Signature_RS", Constants.CONSTANT_Signature);
    ...
    CPRefBand method_descr = class_bands.newCPRefBand("method_descr", MDELTA5, Constants.CONSTANT_NameandType);
    ...
    CPRefBand method_Exceptions_RC = method_attr_bands.newCPRefBand("method_Exceptions_RC", Constants.CONSTANT_Class);
    CPRefBand method_Signature_RS = method_attr_bands.newCPRefBand("method_Signature_RS", Constants.CONSTANT_Signature);
    ...
    CPRefBand method_MethodParameters_name_RUN = method_attr_bands.newCPRefBand("method_MethodParameters_name_RUN", UNSIGNED5, Constants.CONSTANT_Utf8, NULL_IS_OK);
    ...
    CPRefBand class_SourceFile_RUN = class_attr_bands.newCPRefBand("class_SourceFile_RUN", UNSIGNED5, Constants.CONSTANT_Utf8, NULL_IS_OK);
    CPRefBand class_EnclosingMethod_RC = class_attr_bands.newCPRefBand("class_EnclosingMethod_RC", Constants.CONSTANT_Class);
    CPRefBand class_EnclosingMethod_RDN = class_attr_bands.newCPRefBand("class_EnclosingMethod_RDN", UNSIGNED5, Constants.CONSTANT_NameandType, NULL_IS_OK);
    CPRefBand class_Signature_RS = class_attr_bands.newCPRefBand("class_Signature_RS", Constants.CONSTANT_Signature);
    ...
    CPRefBand class_InnerClasses_RC = class_attr_bands.newCPRefBand("class_InnerClasses_RC", Constants.CONSTANT_Class);
    IntBand   class_InnerClasses_F = class_attr_bands.newIntBand("class_InnerClasses_F");
    CPRefBand class_InnerClasses_outer_RCN = class_attr_bands.newCPRefBand("class_InnerClasses_outer_RCN", UNSIGNED5, Constants.CONSTANT_Class, NULL_IS_OK);
    CPRefBand class_InnerClasses_name_RUN = class_attr_bands.newCPRefBand("class_InnerClasses_name_RUN", UNSIGNED5, Constants.CONSTANT_Utf8, NULL_IS_OK);
    ...
    CPRefBand code_handler_class_RCN = code_bands.newCPRefBand("code_handler_class_RCN", UNSIGNED5, Constants.CONSTANT_Class, NULL_IS_OK);
    ...
    CPRefBand code_StackMapTable_RC = stackmap_bands.newCPRefBand("code_StackMapTable_RC", Constants.CONSTANT_Class);
    ...
    CPRefBand code_LocalVariableTable_name_RU = code_attr_bands.newCPRefBand("code_LocalVariableTable_name_RU", Constants.CONSTANT_Utf8);
    CPRefBand code_LocalVariableTable_type_RS = code_attr_bands.newCPRefBand("code_LocalVariableTable_type_RS", Constants.CONSTANT_Signature);
    ...
    CPRefBand code_LocalVariableTypeTable_name_RU = code_attr_bands.newCPRefBand("code_LocalVariableTypeTable_name_RU", Constants.CONSTANT_Utf8);
    CPRefBand code_LocalVariableTypeTable_type_RS = code_attr_bands.newCPRefBand("code_LocalVariableTypeTable_type_RS", Constants.CONSTANT_Signature);
    ...
    // ldc* operands:
    CPRefBand bc_intref = bc_bands.newCPRefBand("bc_intref", DELTA5, Constants.CONSTANT_Integer);
    CPRefBand bc_floatref = bc_bands.newCPRefBand("bc_floatref", DELTA5, Constants.CONSTANT_Float);
    CPRefBand bc_longref = bc_bands.newCPRefBand("bc_longref", DELTA5, Constants.CONSTANT_Long);
    CPRefBand bc_doubleref = bc_bands.newCPRefBand("bc_doubleref", DELTA5, Constants.CONSTANT_Double);
    CPRefBand bc_stringref = bc_bands.newCPRefBand("bc_stringref", DELTA5, Constants.CONSTANT_String);
    CPRefBand bc_loadablevalueref = bc_bands.newCPRefBand("bc_loadablevalueref", DELTA5, Constants.CONSTANT_LoadableValue);

    // nulls produced by bc_classref are taken to mean the current class
    CPRefBand bc_classref = bc_bands.newCPRefBand("bc_classref", UNSIGNED5, Constants.CONSTANT_Class, NULL_IS_OK);   // new, *anew*, c*cast, i*of, ldc
    CPRefBand bc_fieldref = bc_bands.newCPRefBand("bc_fieldref", DELTA5, Constants.CONSTANT_Fieldref);   // get*, put*
    CPRefBand bc_methodref = bc_bands.newCPRefBand("bc_methodref", Constants.CONSTANT_Methodref); // invoke[vs]*
    CPRefBand bc_imethodref = bc_bands.newCPRefBand("bc_imethodref", DELTA5, Constants.CONSTANT_InterfaceMethodref); // invokeinterface
    CPRefBand bc_indyref = bc_bands.newCPRefBand("bc_indyref", DELTA5, Constants.CONSTANT_InvokeDynamic); // invokedynamic

    // _self_linker_op family
    CPRefBand bc_thisfield = bc_bands.newCPRefBand("bc_thisfield", Constants.CONSTANT_None);     // any field within cur. class
    CPRefBand bc_superfield = bc_bands.newCPRefBand("bc_superfield", Constants.CONSTANT_None);   // any field within superclass
    CPRefBand bc_thismethod = bc_bands.newCPRefBand("bc_thismethod", Constants.CONSTANT_None);   // any method within cur. class
    CPRefBand bc_supermethod = bc_bands.newCPRefBand("bc_supermethod", Constants.CONSTANT_None); // any method within superclass
    // bc_invokeinit family:
    IntBand   bc_initref = bc_bands.newIntBand("bc_initref");
    // escapes
    CPRefBand bc_escref = bc_bands.newCPRefBand("bc_escref", Constants.CONSTANT_All);
    ...
    CPRefBand file_name = file_bands.newCPRefBand("file_name", Constants.CONSTANT_Utf8);
    ...
    protected void setBandIndex(CPRefBand b, byte which) {
        ...
        if (which == Constants.CONSTANT_FieldSpecific) {
    ...
    protected MultiBand[] metadataBands = new MultiBand[Constants.ATTR_CONTEXT_LIMIT];
    {
        metadataBands[Constants.ATTR_CONTEXT_CLASS] = class_metadata_bands;
        metadataBands[Constants.ATTR_CONTEXT_FIELD] = field_metadata_bands;
        metadataBands[Constants.ATTR_CONTEXT_METHOD] = method_metadata_bands;
    }
    // Table of bands which contains type_metadata (TypeAnnotations)
    protected MultiBand[] typeMetadataBands = new MultiBand[Constants.ATTR_CONTEXT_LIMIT];
    {
        typeMetadataBands[Constants.ATTR_CONTEXT_CLASS] = class_type_metadata_bands;
        typeMetadataBands[Constants.ATTR_CONTEXT_FIELD] = field_type_metadata_bands;
        typeMetadataBands[Constants.ATTR_CONTEXT_METHOD] = method_type_metadata_bands;
        typeMetadataBands[Constants.ATTR_CONTEXT_CODE]   = code_type_metadata_bands;
    }
    ...
    public int[] attrIndexLimit = new int[Constants.ATTR_CONTEXT_LIMIT];
    // Each index limit is either 32 or 63, depending on AO_HAVE_XXX_FLAGS_HI.

    // Which flag bits are taken over by attributes?
    protected long[] attrFlagMask = new long[Constants.ATTR_CONTEXT_LIMIT];
    // Which flag bits have been taken over explicitly?
    protected long[] attrDefSeen = new long[Constants.ATTR_CONTEXT_LIMIT];

    // What pseudo-attribute bits are there to watch for?
    protected int[] attrOverflowMask = new int[Constants.ATTR_CONTEXT_LIMIT];
    ...
    protected List<List<Attribute.Layout>> attrDefs =
            new FixedList<>(Constants.ATTR_CONTEXT_LIMIT);
    {
        for (int i = 0; i < Constants.ATTR_CONTEXT_LIMIT; i++) {
        ...
        predefineAttribute(Constants.CLASS_ATTR_InnerClasses, Constants.ATTR_CONTEXT_CLASS, null,
                           "InnerClasses", "");
        assert(attrInnerClassesEmpty == Package.attrInnerClassesEmpty);
        predefineAttribute(Constants.CLASS_ATTR_SourceFile, Constants.ATTR_CONTEXT_CLASS,
                           new Band[] { class_SourceFile_RUN },
                           "SourceFile", "RUNH");
        predefineAttribute(Constants.CLASS_ATTR_EnclosingMethod, Constants.ATTR_CONTEXT_CLASS,
                           new Band[] {
        ...
        predefineAttribute(Constants.CLASS_ATTR_ClassFile_version, Constants.ATTR_CONTEXT_CLASS,
                           new Band[] {
                               class_ClassFile_version_minor_H,
                               class_ClassFile_version_major_H
                           },
                           ".ClassFile.version", "HH");
        predefineAttribute(Constants.X_ATTR_Signature, Constants.ATTR_CONTEXT_CLASS,
                           new Band[] { class_Signature_RS },
                           "Signature", "RSH");
        predefineAttribute(Constants.X_ATTR_Deprecated, Constants.ATTR_CONTEXT_CLASS, null,
                           "Deprecated", "");
        //predefineAttribute(X_ATTR_Synthetic, ATTR_CONTEXT_CLASS, null,
        //                 "Synthetic", "");
        predefineAttribute(Constants.X_ATTR_OVERFLOW, Constants.ATTR_CONTEXT_CLASS, null,
                           ".Overflow", "");
        attrConstantValue =
        predefineAttribute(Constants.FIELD_ATTR_ConstantValue, Constants.ATTR_CONTEXT_FIELD,
                           new Band[] { field_ConstantValue_KQ },
                           "ConstantValue", "KQH");
        predefineAttribute(Constants.X_ATTR_Signature, Constants.ATTR_CONTEXT_FIELD,
                           new Band[] { field_Signature_RS },
                           "Signature", "RSH");
        predefineAttribute(Constants.X_ATTR_Deprecated, Constants.ATTR_CONTEXT_FIELD, null,
                           "Deprecated", "");
        //predefineAttribute(X_ATTR_Synthetic, ATTR_CONTEXT_FIELD, null,
        //                 "Synthetic", "");
        predefineAttribute(Constants.X_ATTR_OVERFLOW, Constants.ATTR_CONTEXT_FIELD, null,
                           ".Overflow", "");
        attrCodeEmpty =
        predefineAttribute(Constants.METHOD_ATTR_Code, Constants.ATTR_CONTEXT_METHOD, null,
                           "Code", "");
        predefineAttribute(Constants.METHOD_ATTR_Exceptions, Constants.ATTR_CONTEXT_METHOD,
                           new Band[] {
                               method_Exceptions_N,
                               method_Exceptions_RC
                           },
                           "Exceptions", "NH[RCH]");
        predefineAttribute(Constants.METHOD_ATTR_MethodParameters, Constants.ATTR_CONTEXT_METHOD,
                           new Band[]{
        ...
        predefineAttribute(Constants.X_ATTR_Signature, Constants.ATTR_CONTEXT_METHOD,
                           new Band[] { method_Signature_RS },
                           "Signature", "RSH");
        predefineAttribute(Constants.X_ATTR_Deprecated, Constants.ATTR_CONTEXT_METHOD, null,
                           "Deprecated", "");
        //predefineAttribute(X_ATTR_Synthetic, ATTR_CONTEXT_METHOD, null,
        //                 "Synthetic", "");
        predefineAttribute(Constants.X_ATTR_OVERFLOW, Constants.ATTR_CONTEXT_METHOD, null,
                           ".Overflow", "");
        ...
        for (int ctype = 0; ctype < Constants.ATTR_CONTEXT_LIMIT; ctype++) {
            ...
            if (ctype != Constants.ATTR_CONTEXT_CODE) {
                ...
                predefineAttribute(Constants.X_ATTR_RuntimeVisibleAnnotations,
                                   Constants.ATTR_CONTEXT_NAME[ctype]+"_RVA_",
                                   xxx_metadata_bands,
                                   Attribute.lookup(null, ctype,
                                                    "RuntimeVisibleAnnotations"));
                predefineAttribute(Constants.X_ATTR_RuntimeInvisibleAnnotations,
                                   Constants.ATTR_CONTEXT_NAME[ctype]+"_RIA_",
                                   xxx_metadata_bands,
                                   Attribute.lookup(null, ctype,
                                                    "RuntimeInvisibleAnnotations"));

                if (ctype == Constants.ATTR_CONTEXT_METHOD) {
                    predefineAttribute(Constants.METHOD_ATTR_RuntimeVisibleParameterAnnotations,
                                       "method_RVPA_", xxx_metadata_bands,
                                       Attribute.lookup(null, ctype,
                                       "RuntimeVisibleParameterAnnotations"));
                    predefineAttribute(Constants.METHOD_ATTR_RuntimeInvisibleParameterAnnotations,
                                       "method_RIPA_", xxx_metadata_bands,
                                       Attribute.lookup(null, ctype,
                                       "RuntimeInvisibleParameterAnnotations"));
                    predefineAttribute(Constants.METHOD_ATTR_AnnotationDefault,
            ...
            predefineAttribute(Constants.X_ATTR_RuntimeVisibleTypeAnnotations,
                    Constants.ATTR_CONTEXT_NAME[ctype] + "_RVTA_",
                    xxx_type_metadata_bands,
                    Attribute.lookup(null, ctype,
                    "RuntimeVisibleTypeAnnotations"));
            predefineAttribute(Constants.X_ATTR_RuntimeInvisibleTypeAnnotations,
                    Constants.ATTR_CONTEXT_NAME[ctype] + "_RITA_",
                    xxx_type_metadata_bands,
                    Attribute.lookup(null, ctype,
        ...
        Attribute.Layout stackMapDef = Attribute.lookup(null, Constants.ATTR_CONTEXT_CODE, "StackMapTable").layout();
        predefineAttribute(Constants.CODE_ATTR_StackMapTable, Constants.ATTR_CONTEXT_CODE,
                           stackmap_bands.toArray(),
                           stackMapDef.name(), stackMapDef.layout());

        predefineAttribute(Constants.CODE_ATTR_LineNumberTable, Constants.ATTR_CONTEXT_CODE,
                           new Band[] {
        ...
        predefineAttribute(Constants.CODE_ATTR_LocalVariableTable, Constants.ATTR_CONTEXT_CODE,
                           new Band[] {
        ...
        predefineAttribute(Constants.CODE_ATTR_LocalVariableTypeTable, Constants.ATTR_CONTEXT_CODE,
                           new Band[] {
        ...
        predefineAttribute(Constants.X_ATTR_OVERFLOW, Constants.ATTR_CONTEXT_CODE, null,
                           ".Overflow", "");
        ...
        for (int i = 0; i < Constants.ATTR_CONTEXT_LIMIT; i++) {
            attrOverflowMask[i] = (1<<Constants.X_ATTR_OVERFLOW);
            attrIndexLimit[i] = 0;  // will make a final decision later
        }
        attrClassFileVersionMask = (1<<Constants.CLASS_ATTR_ClassFile_version);
    }
    ...
    private void adjustToClassVersion() throws IOException {
        if (getHighestClassVersion().lessThan(Constants.JAVA6_MAX_CLASS_VERSION)) {
            ...
            undefineAttribute(Constants.CODE_ATTR_StackMapTable, Constants.ATTR_CONTEXT_CODE);
        }
    }
    ...
    protected boolean haveFlagsHi(int ctype) {
        int mask = 1<<(Constants.LG_AO_HAVE_XXX_FLAGS_HI+ctype);
        switch (ctype) {
        case Constants.ATTR_CONTEXT_CLASS:
            assert(mask == Constants.AO_HAVE_CLASS_FLAGS_HI); break;
        case Constants.ATTR_CONTEXT_FIELD:
            assert(mask == Constants.AO_HAVE_FIELD_FLAGS_HI); break;
        case Constants.ATTR_CONTEXT_METHOD:
            assert(mask == Constants.AO_HAVE_METHOD_FLAGS_HI); break;
        case Constants.ATTR_CONTEXT_CODE:
            assert(mask == Constants.AO_HAVE_CODE_FLAGS_HI); break;
        default:
            assert(false);
        }
    ...
    protected void adjustSpecialAttrMasks() {
        ...
        attrClassFileVersionMask &= ~ attrDefSeen[Constants.ATTR_CONTEXT_CLASS];
    ...
    void undefineAttribute(int index, int ctype) {
        if (verbose > 1) {
            System.out.println("Removing predefined "+Constants.ATTR_CONTEXT_NAME[ctype]+
    ...
    protected MultiBand[] attrBands = new MultiBand[Constants.ATTR_CONTEXT_LIMIT];
    {
        attrBands[Constants.ATTR_CONTEXT_CLASS] = class_attr_bands;
        attrBands[Constants.ATTR_CONTEXT_FIELD] = field_attr_bands;
        attrBands[Constants.ATTR_CONTEXT_METHOD] = method_attr_bands;
        attrBands[Constants.ATTR_CONTEXT_CODE] = code_attr_bands;
    }
    ...
    protected CPRefBand getCPRefOpBand(int bc) {
        ...
        case Constants.CONSTANT_LoadableValue:
            switch (bc) {
            case Constants._ildc: case Constants._ildc_w:
                return bc_intref;
            case Constants._fldc: case Constants._fldc_w:
                return bc_floatref;
            case Constants._lldc2_w:
                return bc_longref;
            case Constants._dldc2_w:
                return bc_doubleref;
            case Constants._sldc: case Constants._sldc_w:
                return bc_stringref;
            case Constants._cldc: case Constants._cldc_w:
                return bc_classref;
    ...
    protected CPRefBand selfOpRefBand(int self_bc) {
        ...
        int idx = (self_bc - Constants._self_linker_op);
    ...
    private static void printCDecl(Band b) {
        ...
        if (b instanceof CPRefBand) {
            ConstantPool.Index ix = ((CPRefBand)b).index;
    ...
    static void printArrayTo(PrintStream ps, ConstantPool.Entry[] cpMap, int start, int end, boolean showTags) {
        StringBuffer buf = new StringBuffer();
        int len = end-start;
        for (int i = 0; i < len; i++) {
            ConstantPool.Entry e = cpMap[start+i];
    ...
    protected static int[] realloc(int[] a, int len) {
        if (len == 0)  return Constants.noInts;
    ...
    protected static byte[] realloc(byte[] a, int len) {
        if (len == 0)  return Constants.noBytes;
