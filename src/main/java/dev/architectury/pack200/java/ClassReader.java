...
class ClassReader {
    ...
    private ConstantPool.Entry checkTag(ConstantPool.Entry e, byte tag) throws ClassFormatException {
    ...
    private ConstantPool.Entry checkTag(ConstantPool.Entry e, byte tag, boolean nullOK) throws ClassFormatException {
    ...
    private ConstantPool.Utf8Entry readUtf8Ref() throws IOException {
        return (ConstantPool.Utf8Entry) readRef(Constants.CONSTANT_Utf8);
    }

    private ConstantPool.ClassEntry readClassRef() throws IOException {
        return (ConstantPool.ClassEntry) readRef(Constants.CONSTANT_Class);
    }

    private ConstantPool.ClassEntry readClassRefOrNull() throws IOException {
        return (ConstantPool.ClassEntry) readRefOrNull(Constants.CONSTANT_Class);
    }
    ...
    private ConstantPool.SignatureEntry readSignatureRef() throws IOException {
        ...
        ConstantPool.Entry e = readRef(Constants.CONSTANT_Signature);
        return (e != null && e.getTag() == Constants.CONSTANT_Utf8)
                ? ConstantPool.getSignatureEntry(e.stringValue())
                : (ConstantPool.SignatureEntry) e;
    }
    ...
    void read() throws IOException {
        ...
        try {
            ...
            readAttributes(Constants.ATTR_CONTEXT_CLASS, cls);
        ...
        if (bad != null) {
            throw new Attribute.FormatException
                ("classfile version too "+bad+": "
                 ...
                 Constants.ATTR_CONTEXT_CLASS, "version", "pass");
        }
    }
    ...
    private static final ConstantPool.Entry INVALID_ENTRY = new ConstantPool.Entry((byte) -1) {
        @Override
    ...
    void readConstantPool() throws IOException {
        ...
        while (fptr > 0) {
            ...
            for (int fi = 0; fi < flimit; ) {
                ...
                case Constants.CONSTANT_Methodref:
                case Constants.CONSTANT_InterfaceMethodref:
                    ConstantPool.ClassEntry mclass = (ConstantPool.ClassEntry)      checkTag(cpMap[ref],  Constants.CONSTANT_Class);
                    ConstantPool.DescriptorEntry mdescr = (ConstantPool.DescriptorEntry) checkTag(cpMap[ref2], Constants.CONSTANT_NameandType);
                    cpMap[cpi] = ConstantPool.getMemberEntry((byte)tag, mclass, mdescr);
                    break;
                case Constants.CONSTANT_NameandType:
                    ConstantPool.Utf8Entry mname = (ConstantPool.Utf8Entry) checkTag(cpMap[ref],  Constants.CONSTANT_Utf8);
                    ConstantPool.Utf8Entry mtype = (ConstantPool.Utf8Entry) checkTag(cpMap[ref2], Constants.CONSTANT_Signature);
                    cpMap[cpi] = ConstantPool.getDescriptorEntry(mname, mtype);
                    break;
                case Constants.CONSTANT_MethodType:
                    cpMap[cpi] = ConstantPool.getMethodTypeEntry((ConstantPool.Utf8Entry) checkTag(cpMap[ref], Constants.CONSTANT_Signature));
                    break;
                case Constants.CONSTANT_MethodHandle:
                    byte refKind = (byte)(-1 ^ ref);
                    ConstantPool.MemberEntry memRef = (ConstantPool.MemberEntry) checkTag(cpMap[ref2], Constants.CONSTANT_AnyMember);
                    cpMap[cpi] = ConstantPool.getMethodHandleEntry(refKind, memRef);
                    break;
                case Constants.CONSTANT_InvokeDynamic:
                    ConstantPool.DescriptorEntry idescr = (ConstantPool.DescriptorEntry) checkTag(cpMap[ref2], Constants.CONSTANT_NameandType);
    ...
    private /*non-static*/
    class UnresolvedEntry extends ConstantPool.Entry {
        ...
        ConstantPool.Entry resolve() {
            Package.Class cls = ClassReader.this.cls;
            ConstantPool.Entry res;
            switch (tag) {
            case Constants.CONSTANT_InvokeDynamic:
                ConstantPool.BootstrapMethodEntry iboots = cls.bootstrapMethods.get((Integer) refsOrIndexes[0]);
                ConstantPool.DescriptorEntry idescr = (ConstantPool.DescriptorEntry) refsOrIndexes[1];
    ...
    private void fixUnresolvedEntries() {
        ...
        ConstantPool.Entry[] cpMap = cls.getCPMap();
    ...
    void readHeader() throws IOException {
        ...
        cls.interfaces = new ConstantPool.ClassEntry[ni];
        for (int i = 0; i < ni; i++) {
    ...
    void readMember(boolean doMethod) throws IOException {
        ...
        ConstantPool.Utf8Entry mname = readUtf8Ref();
        ConstantPool.SignatureEntry mtype = readSignatureRef();
        ...
        readAttributes(!doMethod ? Constants.ATTR_CONTEXT_FIELD : Constants.ATTR_CONTEXT_METHOD,
                       m);
    }
    void readAttributes(int ctype, Attribute.Holder h) throws IOException {
        ...
        for (int i = 0; i < na; i++) {
            ...
            boolean isStackMap = (ctype == Constants.ATTR_CONTEXT_CODE
    ...
    void readCode(Code code) throws IOException {
        ...
        readAttributes(Constants.ATTR_CONTEXT_CODE, code);
    }

    void readBootstrapMethods(Package.Class cls) throws IOException {
        ConstantPool.BootstrapMethodEntry[] bsms = new ConstantPool.BootstrapMethodEntry[readUnsignedShort()];
        for (int i = 0; i < bsms.length; i++) {
            ConstantPool.MethodHandleEntry bsmRef = (ConstantPool.MethodHandleEntry) readRef(Constants.CONSTANT_MethodHandle);
            ConstantPool.Entry[] argRefs = new ConstantPool.Entry[readUnsignedShort()];
            for (int j = 0; j < argRefs.length; j++) {
                argRefs[j] = readRef(Constants.CONSTANT_LoadableValue);
            }
    ...
    void readInnerClasses(Package.Class cls) throws IOException {
        ...
        ArrayList<Package.InnerClass> ics = new ArrayList<>(nc);
        for (int i = 0; i < nc; i++) {
            Package.InnerClass ic =
                new Package.InnerClass(readClassRef(),
                               readClassRefOrNull(),
                               (ConstantPool.Utf8Entry)readRefOrNull(Constants.CONSTANT_Utf8),
                               readUnsignedShort());
            ics.add(ic);
        }
