...
import dev.architectury.pack200.java.ConstantPool.ClassEntry;
import dev.architectury.pack200.java.ConstantPool.DescriptorEntry;
import dev.architectury.pack200.java.ConstantPool.BootstrapMethodEntry;
import dev.architectury.pack200.java.ConstantPool.Index;
import dev.architectury.pack200.java.ConstantPool.LiteralEntry;
import dev.architectury.pack200.java.ConstantPool.Utf8Entry;
import dev.architectury.pack200.java.ConstantPool.Entry;
...
class Package {
    ...
    final int magic = Constants.JAVA_PACKAGE_MAGIC;

    int default_modtime = Constants.NO_MODTIME;
    ...
    public Package(Version minClassVersion, Version maxClassVersion, Version packageVersion) {
        ...
        this.minClassVersion = minClassVersion == null
                ? Constants.JAVA_MIN_CLASS_VERSION
                : minClassVersion;
        this.maxClassVersion = maxClassVersion == null
                ? Constants.JAVA_MAX_CLASS_VERSION
                : maxClassVersion;
    ...
    static {
        Map<Attribute.Layout, Attribute> ad = new HashMap<>(3);
        attrCodeEmpty = Attribute.define(ad, Constants.ATTR_CONTEXT_METHOD,
                                         "Code", "").layout();
        attrBootstrapMethodsEmpty = Attribute.define(ad, Constants.ATTR_CONTEXT_CLASS,
                                                     "BootstrapMethods", "").layout();
        attrInnerClassesEmpty = Attribute.define(ad, Constants.ATTR_CONTEXT_CLASS,
                                                 "InnerClasses", "").layout();
        attrSourceFileSpecial = Attribute.define(ad, Constants.ATTR_CONTEXT_CLASS,
    ...
    private void setHighestClassVersion() {
        if (observedHighestClassVersion != null)
            return;
        Version res = Constants.JAVA_MIN_CLASS_VERSION;  // initial low value
    ...
    class Class extends Attribute.Holder implements Comparable<Class> {
        ...
        private void transformSourceFile(boolean minimize) {
            ...
            olda.visitRefs(this, Constants.VRM_PACKAGE, ref);
            Utf8Entry sfName = (Utf8Entry) ref.get(0);
            Attribute a = olda;
            if (sfName == null) {
                if (minimize) {
                    // A pair of zero bytes.  Cannot use predef. layout.
                    a = Attribute.find(Constants.ATTR_CONTEXT_CLASS, "SourceFile", "H");
        ...
        public List<InnerClass> computeGloballyImpliedICs() {
            ...
            {   // This block temporarily displaces this.innerClasses.
                ...
                visitRefs(Constants.VRM_CLASSIC, cpRefs);
    ...
    public final class File implements Comparable<File> {
        ...
        public boolean isClassStub() {
            return (options & Constants.FO_IS_CLASS_STUB) != 0;
        }
        ...
        public boolean isTrivialClassStub() {
            return isClassStub()
                ...
                && (modtime == Constants.NO_MODTIME || modtime == default_modtime)
                && (options &~ Constants.FO_IS_CLASS_STUB) == 0;
        }
        ...
        public String toString() {
            return nameString+"{"
                +(isClassStub()?"*":"")
                +(BandStructure.testBit(options,Constants.FO_DEFLATE_HINT)?"@":"")
                +(modtime==Constants.NO_MODTIME?"":"M"+modtime)
    ...
    File newStub(String classFileNameString) {
        ...
        stub.options |= Constants.FO_IS_CLASS_STUB;
        stub.prepend = null;
    ...
    class InnerClass implements Comparable<InnerClass> {
        ...
        protected void visitRefs(int mode, Collection<Entry> refs) {
            refs.add(thisClass);
            if (mode == Constants.VRM_CLASSIC || !predictable) {
...
*/
    ...
    void buildGlobalConstantPool(Set<Entry> requiredEntries) {
        ...
        visitRefs(Constants.VRM_PACKAGE, requiredEntries);
