...
class PackageWriter extends BandStructure {
    ...
    void chooseDefaultPackageVersion() throws IOException {
        ...
        if (highV.lessThan(Constants.JAVA6_MAX_CLASS_VERSION)) {
            // There are only old classfiles in this segment or resources
            packageVersion = Constants.JAVA5_PACKAGE_VERSION;
        } else if (highV.equals(Constants.JAVA6_MAX_CLASS_VERSION) ||
                (highV.equals(Constants.JAVA7_MAX_CLASS_VERSION) && !pkg.cp.haveExtraTags())) {
        ...
        } else if (highV.equals(Constants.JAVA7_MAX_CLASS_VERSION)) {
    ...
    void checkVersion() throws IOException {
        ...
        if (packageVersion.lessThan(Constants.JAVA7_PACKAGE_VERSION)) {
            ...
            if (testBit(archiveOptions, Constants.AO_HAVE_CP_EXTRAS)) {
                throw new IOException("Format bits for Java 7 must be zero in previous releases");
            }
        }
        if (testBit(archiveOptions, Constants.AO_UNUSED_MBZ)) {
    ...
    void setArchiveOptions() {
        ...
        for (Package.File file : pkg.files) {
            ...
            if (minModtime == Constants.NO_MODTIME) {
        ...
        if (pkg.default_modtime == Constants.NO_MODTIME) {
            // Make everything else be a positive offset from here.
            pkg.default_modtime = minModtime;
        }
        if (minModtime != Constants.NO_MODTIME && minModtime != maxModtime) {
            // Put them into a band.
            archiveOptions |= Constants.AO_HAVE_FILE_MODTIME;
        }
        // If the archive deflation is set do not bother with each file.
        if (!testBit(archiveOptions,Constants.AO_DEFLATE_HINT) && minOptions != -1) {
            if (testBit(minOptions, Constants.FO_DEFLATE_HINT)) {
                // Every file has the deflate_hint set.
                // Set it for the whole archive, and omit options.
                archiveOptions |= Constants.AO_DEFLATE_HINT;
                minOptions -= Constants.FO_DEFLATE_HINT;
                maxOptions -= Constants.FO_DEFLATE_HINT;
            }
            pkg.default_options |= minOptions;
            if (minOptions != maxOptions
                || minOptions != pkg.default_options) {
                archiveOptions |= Constants.AO_HAVE_FILE_OPTIONS;
            }
        }
        ...
        verCounts.clear();
        if (bestVersion == null)  bestVersion = Constants.JAVA_MIN_CLASS_VERSION;  // degenerate case
        ...
        for (Package.File file : pkg.files) {
            ...
            if (len != (int)len) {
                archiveOptions |= Constants.AO_HAVE_FILE_SIZE_HI;
                if (verbose > 0)
        ...
        if (cost0 > cost1) {
            archiveOptions |= Constants.AO_HAVE_ALL_CODE_FLAGS;
        }
        if (verbose > 0)
    ...
    void writeArchiveHeader() throws IOException {
        ...
        if (!haveSpecial) {
            ...
            if (haveSpecial)
                archiveOptions |= Constants.AO_HAVE_SPECIAL_FORMATS;
        }
        if (haveSpecial)
        ...
        if (!haveFiles) {
            ...
            haveFiles |= (pkg.default_modtime != Constants.NO_MODTIME);
            if (haveFiles)
                archiveOptions |= Constants.AO_HAVE_FILE_HEADERS;
        }
        if (haveFiles)
        ...
        if (!haveNumbers) {
            ...
            if (haveNumbers)
                archiveOptions |= Constants.AO_HAVE_CP_NUMBERS;
        }
        if (haveNumbers)
...
// with a zipped unbanded file.
    ...
    void writeFiles() throws IOException {
        ...
        if (!haveOptions) {
            for (Package.File file : pkg.files) {
                if (file.isClassStub()) {
                    haveOptions = true;
                    options |= Constants.AO_HAVE_FILE_OPTIONS;
        ...
        if (haveSizeHi || haveModtime || haveOptions || !pkg.files.isEmpty()) {
            options |= Constants.AO_HAVE_FILE_HEADERS;
            archiveOptions = options;
        }
        for (Package.File file : pkg.files) {
    ...
    void collectAttributeLayouts() {
        maxFlags = new int[Constants.ATTR_CONTEXT_LIMIT];
        ...
        for (Package.Class cls : pkg.classes) {
            visitAttributeLayoutsIn(Constants.ATTR_CONTEXT_CLASS, cls);
            for (Package.Class.Field f : cls.getFields()) {
                visitAttributeLayoutsIn(Constants.ATTR_CONTEXT_FIELD, f);
            }
            for (Package.Class.Method m : cls.getMethods()) {
                visitAttributeLayoutsIn(Constants.ATTR_CONTEXT_METHOD, m);
                if (m.code != null) {
                    visitAttributeLayoutsIn(Constants.ATTR_CONTEXT_CODE, m.code);
                }
            }
        }
        ...
        for (int i = 0; i < Constants.ATTR_CONTEXT_LIMIT; i++) {
            ...
            if (nl >= TOO_MANY_ATTRS) {  // heuristic
                int mask = 1<<(Constants.LG_AO_HAVE_XXX_FLAGS_HI+i);
                archiveOptions |= mask;
                haveLongFlags = true;
                if (verbose > 0)
        ...
        attrCounts = new int[Constants.ATTR_CONTEXT_LIMIT][];
        for (int i = 0; i < Constants.ATTR_CONTEXT_LIMIT; i++) {
    ...
    void writeAttrDefs() throws IOException {
        ...
        {
            int[] indexForDebug = Arrays.copyOf(attrIndexLimit, Constants.ATTR_CONTEXT_LIMIT);
    ...
    void writeInnerClasses() throws IOException {
        for (Package.InnerClass ic : pkg.getAllInnerClasses()) {
            int flags = ic.flags;
            assert((flags & Constants.ACC_IC_LONG_FORM) == 0);
            if (!ic.predictable) {
    ...
    void writeLocalInnerClasses(Package.Class cls) throws IOException {
        ...
        for(Package.InnerClass ic : localICs) {
            ...
            if (ic.equals(pkg.getGlobalInnerClass(ic.thisClass))) {
                ...
                int flags = ic.flags;
                if (flags == 0)
                    flags = Constants.ACC_IC_LONG_FORM;  // force it to be non-zero
    ...
    void writeClassesAndByteCodes() throws IOException {
        ...
        for (int i = 0; i < classes.length; i++) {
            ...
            writeMembers(cls);
            writeAttrs(Constants.ATTR_CONTEXT_CLASS, cls, cls);
    ...
    void writeMembers(Package.Class cls) throws IOException {
        ...
        for (Package.Class.Field f : fields) {
            ...
            writeAttrs(Constants.ATTR_CONTEXT_FIELD, f, cls);
        }
        ...
        for (Package.Class.Method m : methods) {
            ...
            writeAttrs(Constants.ATTR_CONTEXT_METHOD, m, cls);
    ...
    void writeCodeHeader(Code c) throws IOException {
        ...
        if (sc == LONG_CODE_HEADER || attrsOK)
            writeAttrs(Constants.ATTR_CONTEXT_CODE, c, c.thisClass());
    }
    ...
    void writeAttrs(int ctype,
        ...
        for (Attribute a : h.attributes) {
            ...
            if (index < attrIndexLimit[ctype] && testBit(flagMask, 1L<<index)) {
                ...
                // an overflow attr.
                flagsToAdd |= (1L<<Constants.X_ATTR_OVERFLOW);
                overflowCount += 1;
                if (verbose > 3)
    ...
    private int initOpVariant(Instruction i, ConstantPool.Entry newClass) {
        if (i.getBC() != Constants._invokespecial)  return -1;
        ...
        if (refClass == curClass.thisClass)
            return Constants._invokeinit_op+Constants._invokeinit_self_option;
        if (refClass == curClass.superClass)
            return Constants._invokeinit_op+Constants._invokeinit_super_option;
        if (refClass == newClass)
            return Constants._invokeinit_op+Constants._invokeinit_new_option;
        return -1;
    }
    ...
    private int selfOpVariant(Instruction i) {
        int bc = i.getBC();
        if (!(bc >= Constants._first_linker_op && bc <= Constants._last_linker_op))  return -1;
        ConstantPool.MemberEntry ref = (ConstantPool.MemberEntry) i.getCPRef(curCPMap);
        // do not optimize this case, simply fall back to regular coding
        if ((bc == Constants._invokespecial || bc == Constants._invokestatic) &&
                ref.tagEquals(Constants.CONSTANT_InterfaceMethodref))
            return -1;
        ConstantPool.ClassEntry refClass = ref.classRef;
        int self_bc = Constants._self_linker_op + (bc - Constants._first_linker_op);
        if (refClass == curClass.thisClass)
            return self_bc;
        if (refClass == curClass.superClass)
            return self_bc + Constants._self_linker_super_flag;
        return -1;
    }
    ...
    void writeByteCodes(Code code) throws IOException {
        ...
        for (Instruction i = code.instructionAt(0); i != null; i = i.next()) {
            ...
            if (i.isWide()) {
                ...
                bc_codes.putByte(Constants._wide);
                codeHist[Constants._wide]++;
            }

            int bc = i.getBC();

            // Begin "bc_linker" compression.
            if (bc == Constants._aload_0) {
            ...
            if (init_bc >= 0) {
                if (prevAload) {
                    // get rid of it
                    bc_codes.putByte(Constants._aload_0);
            ...
            if (self_bc >= 0) {
                ...
                boolean isSuper = (self_bc >= Constants._self_linker_op+Constants._self_linker_super_flag);
                boolean isAload = prevAload;
                prevAload = false;  //used up
                if (isAload)
                    self_bc += Constants._self_linker_aload_flag;
                // Write special bytecode.
            ...
            if (ref != null) {
                if (bc == Constants._new)  newClass = ref;
                if (bc == Constants._ldc)  ldcHist[ref.tag]++;
                CPRefBand bc_which;
                int vbc = bc;
                switch (i.getCPTag()) {
                case Constants.CONSTANT_LoadableValue:
                    ...
                    case Constants.CONSTANT_Integer:
                        bc_which = bc_intref;
                        switch (bc) {
                        case Constants._ldc:    vbc = Constants._ildc; break;
                        case Constants._ldc_w:  vbc = Constants._ildc_w; break;
                    ...
                    case Constants.CONSTANT_Float:
                        ...
                        switch (bc) {
                        case Constants._ldc:    vbc = Constants._fldc; break;
                        case Constants._ldc_w:  vbc = Constants._fldc_w; break;
                        default:      assert(false);
                        }
                        break;
                    case Constants.CONSTANT_Long:
                        bc_which = bc_longref;
                        assert(bc == Constants._ldc2_w);
                        vbc = Constants._lldc2_w;
                        break;
                    case Constants.CONSTANT_Double:
                        bc_which = bc_doubleref;
                        assert(bc == Constants._ldc2_w);
                        vbc = Constants._dldc2_w;
                        break;
                    case Constants.CONSTANT_String:
                        bc_which = bc_stringref;
                        switch (bc) {
                        case Constants._ldc:    vbc = Constants._sldc; break;
                        case Constants._ldc_w:  vbc = Constants._sldc_w; break;
                        default:      assert(false);
                        }
                        break;
                    case Constants.CONSTANT_Class:
                        ...
                        switch (bc) {
                        case Constants._ldc:    vbc = Constants._cldc; break;
                        case Constants._ldc_w:  vbc = Constants._cldc_w; break;
                        default:      assert(false);
                        }
                        break;
                    default:
                        ...
                        switch (bc) {
                        case Constants._ldc:    vbc = Constants._qldc; break;
                        case Constants._ldc_w:  vbc = Constants._qldc_w; break;
                        default:      assert(false);
                        }
                    }
                    break;
                ...
                case Constants.CONSTANT_Methodref:
                    if (ref.tagEquals(Constants.CONSTANT_InterfaceMethodref)) {
                        if (bc == Constants._invokespecial)
                            vbc = Constants._invokespecial_int;
                        if (bc == Constants._invokestatic)
                            vbc = Constants._invokestatic_int;
                ...
                if (bc == Constants._multianewarray) {
                ...
                } else if (bc == Constants._invokedynamic) {
                    if (getHighestClassVersion().lessThan(Constants.JAVA7_MAX_CLASS_VERSION)) {
                    ...
                    assert(i.getLength() == ((bc == Constants._ldc)?2:3));
                }
                continue;
            }
        ...
        bc_codes.putByte(Constants._end_marker);
        bc_codes.elementCountForDebug++;
        codeHist[Constants._end_marker]++;
        endCode();
    }
